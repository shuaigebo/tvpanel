	...
	<table border="1" bordercolor="#a0c6e5" style="border-collapse:collapse;">
		...
		<form method="POST">
			...
			<?php
				...
				if($user=='admin'){
				$sql="select status,name,mac,deviceid,model,ip,region,lasttime,exp,author,marks,vpn from chzb_users where status>-1 $searchparam order by $order  limit $recStart,$recCounts";
				}else{
					$sql="select status,name,mac,deviceid,model,ip,region,lasttime,exp,author,marks,vpn from chzb_users where status>-1 and author='$user' $searchparam order by $order limit $recStart,$recCounts";
				}
